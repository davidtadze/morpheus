...
Matrix3::Matrix3(const initializer_list_float init_list) {
...
auto Matrix3::operator()(unsigned int i, unsigned int j) -> float& {
  return n_[j][i];
}

auto Matrix3::operator[](unsigned int i) -> Vector3& {
  return reinterpret_cast<Vector3&>(n_[i]);
}

auto Matrix3::operator*=(const Matrix3& m) -> Matrix3& {
  Matrix3 tmp;
  for (int j = 0; j < 3; ++j) {
    for (int k = 0; k < 3; ++k) {
      for (int i = 0; i < 3; ++i) {
